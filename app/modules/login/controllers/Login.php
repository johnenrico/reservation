...
class Login extends MX_Controller
{
	...
	function index()
	{
		...
		if($_COOKIE["user_token"] != ''){
			...
			$id = $this->general->get_table('customers', array('token' => base64_encode(base64_encode($_COOKIE["user_token"]))), 'id')->row()->id;
			if($id != ''){
	...
	function do_login()
	{
		...
		$is_login = $this->general->get_table('customers', ['username' => $all_post->username, 'password' => base64_encode(base64_encode($all_post->password))], 'id');
		...
		if($is_login->num_rows() > 0){
			...
			if($all_post->remember == "on"){
				...
				$this->general->update_table('customers', array('id' => $is_login->row()->id), array('token' => base64_encode(base64_encode($remember))));
